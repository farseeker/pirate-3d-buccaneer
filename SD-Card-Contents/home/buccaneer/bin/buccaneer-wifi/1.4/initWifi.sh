...
case "$1" in
	...
	PIDS2=`ps auxwww | grep 'dhcpcd wlan1' | grep -v "grep" | awk '{print $2}'`
	...
	if [ -z "$PIDS1" ]; then
		if [ -z "$PIDS2" ]; then
			...
			ip link set dev wlan1 up
        		wpa_supplicant -B -Dwext -i wlan1 -c /etc/wpa_supplicant/wpa_supplicant.conf
        		dhcpcd wlan1
...
#		echo "Wifi already running"
	...
	#echo $PIDS2
		...
		if [ "$PIDS2" ]; then
        		...
        		#LCD_PID=`ps auxwww | grep 'dhcpcd wlan1' | grep -v "grep" | awk '{print $2}'`
        		#kill $LCD_PID
			...
			[ -f /var/run/dhcpcd-wlan1.pid ] && kill `cat /var/run/dhcpcd-wlan1.pid` &> /dev/null
