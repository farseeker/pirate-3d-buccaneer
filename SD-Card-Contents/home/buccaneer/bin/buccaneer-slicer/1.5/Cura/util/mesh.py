...
class printableObject(object):
	...
	def scaleUpTo(self, size, userScale=None):
		...
		#print ("Object's original scale relative to platform: scale = %s" % (scale))

		if userScale is not None:
			scale = (scale * userScale / 100.0)*(0.98)
			#print("After scaling by user: scale = %s" % (scale))
			self.applyMatrix(numpy.matrix([[scale,0,0],[0,scale,0],[0,0,scale]], numpy.float64))
			return

		if (scale > 0 and scale < 1):
			scale = scale * 0.98
			#print "object scaled down"
			self.applyMatrix(numpy.matrix([[scale,0,0],[0,scale,0],[0,0,scale]], numpy.float64))
